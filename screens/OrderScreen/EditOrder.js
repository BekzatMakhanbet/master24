...
class EditOrderScreen extends React.Component {
  static navigationOptions = ({ navigation }) => {
    let { state } = navigation;
    return {
      title: i18n.t('order:editOrder'),
      headerLeft: (
        <TouchableOpacity
  ...
  updateOrder = () => {
    ...
    let body = {
      ...
      specialization: this.state.specialization.id,
      description: this.state.description,
      orderPriceType: this.state.orderPriceType,
    ...
    axios({
      method: 'PATCH',
      url: `${config.url}/api/v1/order/${this.state.id}`,
      headers: {
      ...
      .then(async (res) => {
        this.state.deleteImages.forEach(async (image) => {
          if (image) {
            await axios.delete(`${config.url}/api/v1/image/${image}`, {
              headers: {},
            });
          }
        });
        ...
        if (hasImage) {
          await axios({
            method: 'POST',
            url: `${config.url}/api/v1/image/order/${this.state.id}`,
            headers: {
        ...
        await axios({
          method: 'DELETE',
          url: `${config.url}/api/v1/history/order/${this.state.id}`,
        });
          ...
          {this.state.spinning ? (
            ...
            <View
              style={{
                ...
                width: '100%',
                paddingVertical: 15,
                marginBottom: this.state.description.split('\n').length * 20,
              }}
            >
              <View style={styles.container}>
                <View style={styles.inputContainer}>
                  <View>
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        this.props.navigation.navigate('OneSpecList');
                      }}
                      style={{
                        flexDirection: 'row',
                        width: '100%',
                ...
                >
                  ...
                  >
                    <Text allowFontScaling={false} style={styles.titleTextModal}>
                      {t('simple:edittingWillBeCanceled')}
                    </Text>
...
)(hoistStatics(withTranslation()(EditOrderScreen), EditOrderScreen));
